export default async function handler(req, res) {
  ...
  try {
    ...
    // مهم: هاد التوكن خاصو يتبدّل كل 6 أشهر. إلا وقف الموقع، صيفط لي نعطيك واحد جديد
    ...
    if (type === 'docx-pdf') {
      ...
      tasks['optimize-1'] = { operation: 'optimize', input: 'import-0', preset: 'balanced' };
      tasks['export-1'] = { operation: 'export/url', input: 'optimize-1' };
    } else if (type === 'merge') {
      ...
      tasks['convert-1'] = { operation: 'convert', input: files.map((_, i) => `import-${i}`), output_format: 'pdf' };
      tasks['export-1'] = { operation: 'export/url', input: 'convert-1' };
    } else if (type === 'split') {
    ...
    // هادي هي اللي كتصلح الخطأ ديالك
    if (!jobRes.ok || job.errors) {
      throw new Error(job.message || 'فشل الاتصال بخادم التحويل. تأكد من صحة التوكن');
    }
    if (!job.data) {
      throw new Error('لم يتم استلام بيانات المهمة من السيرفر');
    }
    
    const exportTask = job.data.tasks.find(t => t.operation === 'export/url');
    if (!exportTask) throw new Error('لم يتم إنشاء مهمة التصدير');
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (exportResult.data.status === 'error') throw new Error(exportResult.data.message || 'فشل التحويل');
      await new Promise(r => setTimeout(r, 1500));
    }

    if (!exportResult.data.result ||!exportResult.data.result.files ||!exportResult.data.result.files[0]) {
      throw new Error('تم التحويل ولكن لم يتم العثور على رابط التحميل');
    }
    ...
    console.error(error);
    res.status(500).json({ error: error.message || 'خطأ غير معروف في السيرفر' });
  }
}
